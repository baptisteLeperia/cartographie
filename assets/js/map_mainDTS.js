...
document.addEventListener('dblclick', e => {
    ...
    if (e.button == 0 && isInModificationMode) {
        if (selectedElement != null && getPointedElement(M) == selectedElement)
        {
            ...
            else if (selectedElement.category == "rectangles" || selectedElement.category == "polygons")
            {
                var recpol = selectedElement;

                if (!(modification.action == MOVE))
                {
                    var str="";
                    if (recpol.number!=null || recpol.number!=undefined || recpol.number!="") {
                        str=recpol.number;
                    }
                    var number = prompt("Numéro du carré :", str);
                    if (number == null || number == "")
                    {
                        // do not modify grave number
                    }
